...
class SubscribeController extends Controller
{
    ...
    public function getPrice(Request $request)
    {
        ...
        $zooid = DB::table('zooid')->where('hide', 0)->where('delete', 0)->get();
        $ZooIDPrice = [];
        $ZooIDPrice['BYN'] = 0;
        $ZooIDPrice['RUB'] = 0;
        if ($zooid) {
        ...
        $concierge = DB::table('concierge')->where('hide', 0)->where('delete', 0)->get();
        $ConciergePrice = [];
    ...
    public function bay(Request $request)
    {
        ...
        $time = intval($period['period'][0]);
        if (isset($userID)) {
            if (intval($period['type']) == 1) {
                $check = DB::table('sub_zooid_user')->where('user', $userID)->where('status', 3)->where(
                    'end',
                    '>=',
                    DB::raw('now()')
                )->first();
...
//                $check = DB::table('sub_concierge_user')->where('user', $userID)->where('status', 3)->where(
//                    'end',
//                    '>=',
//                    DB::raw('now()')
//                )->first();
...
//                } else {
//                    $subscriptionData = DB::table('sub_concierge')->where('month', $period['period'][0])->where(
//                        'status',
//                        1
//                    )->first();
//                    if ($subscriptionData) {
...
//                $check = DB::table('sub_zoopolis_user')->where('user', $userID)->where('status', 3)->where(
//                    'end',
//                    '>=',
//                    DB::raw('now()')
//                )->first();
...
//                } else {
//                    $subscriptionData = DB::table('sub_zoopolis')->where('month', $period['period'][0])->where(
//                        'status',
//                        1
//                    )->first();
//                    if ($subscriptionData) {
...
//            };
    ...
    public function status(Request $request)
    {
        ...
        if (isset($userID)) {
            $zooid = DB::table('sub_zooid_user')->where('user', $userID)->where('status', 3)->where(
                'end',
                '>=',
                DB::raw('now()')
            )->first();
            if ($zooid) {
                ...
                $zooidConf = DB::table('zooid')->where('hide', 0)->where('delete', 0)->get();
                if ($zooidConf) {
            ...
            $concierge = DB::table('sub_concierge_user')->where('user', $userID)->where(
                'end',
                '>=',
                DB::raw('now()')
            )->first();
            if ($concierge) {
                ...
                $configConcirge = [];
                $userConciergeConfigs = json_decode($concierge->config);
                foreach ($userConciergeConfigs as $i => $data) {
                    if (intval($data) > 1) {
                        array_push($configConcirge, intval($data));
                    }
                }

                $zooidConf = DB::table('concierge')->whereIn('id', $configConcirge)->where('delete', 0)->get();
                if ($zooidConf) {
                    foreach ($zooidConf as $i => $data) {
                        $return['concierge']['list'][$i] = $data->name;
                    }
                }
            ...
            $zoopolis = DB::table('sub_zoopolis_user')->where('user', $userID)->where(
                'end',
                '>=',
                DB::raw('now()')
            )->first();
            if ($zoopolis) {
                ...
                $return['zoopolis']['list'] = [];

                $configZooPolis = [];
                $userZooPolisConfigs = json_decode($zoopolis->config);
                foreach ($userZooPolisConfigs as $i => $data) {
                    if (intval($data) > 1) {
                        array_push($configZooPolis, intval($data));
                    }
                }

                $zooidConf = DB::table('zoopolis')->whereIn('id', $configZooPolis)->where('delete', 0)->get();
                if ($zooidConf) {
                    foreach ($zooidConf as $i => $data) {
                        $return['zoopolis']['list'][$i] = $data->name;
                    }
                }
            ...
            $zoopolisConfigs = DB::table('zoopolis')->where('hide', 0)->where('delete', 0)->get();
            foreach ($zoopolisConfigs as $i => $data) {
                $return['configs']['zoopolis'][$i]['id'] = $data->id;
                $return['configs']['zoopolis'][$i]['name'] = $data->name;
                $return['configs']['zoopolis'][$i]['BYN'] = $data->BYN;
                $return['configs']['zoopolis'][$i]['RUB'] = $data->RUB;
            }

            $zooIDUserConfigs = DB::table('zooid_userconfig')->where('user', $userID)->first();

            if ($zooIDUserConfigs) {
                $return['configs']['zooiduser'] = json_decode($zooIDUserConfigs->config);
            }

            $zooPolisUserConfigs = DB::table('zoopolis_userconfig')->where('user', $userID)->first();

            if ($zooPolisUserConfigs) {
                $return['configs']['zoopolisuser'] = json_decode($zooPolisUserConfigs->config);
            }

            $conciergeUserConfigs = DB::table('concierge_userconfig')->where('user', $userID)->first();

            if ($conciergeUserConfigs) {
                $return['configs']['conciergeuser'] = json_decode($conciergeUserConfigs->config);
            }
        } else {
            $return['err'] = "Необходима автворизация";
        }
    ...
    public function configZooid(Request $request)
    {
        ...
        $getUser = User::getUser($request);
        $userID = $getUser['id'];
        DB::table('zooid_userconfig')->where('user', $userID)->delete();
        $zooid = implode(',', $request->input('zooid'));
        $zooid = array_map('intval', explode(',', $zooid));
        DB::table('zooid_userconfig')->insertGetId([
            'user' => $userID,
    ...
    public function configZooPolis(Request $request)
    {
        $return = [];
        $getUser = User::getUser($request);
        $userID = $getUser['id'];
        DB::table('zoopolis_userconfig')->where('user', $userID)->delete();
        $zooid = implode(',', $request->input('zoopolis'));
        $zooid = array_map('intval', explode(',', $zooid));
        DB::table('zoopolis_userconfig')->insertGetId([
            'user' => $userID,
            'config' => json_encode($zooid)
        ]);
    ...
    public function configConcierge(Request $request)
    {
        $return = [];
        $getUser = User::getUser($request);
        $userID = $getUser['id'];
        DB::table('concierge_userconfig')->where('user', $userID)->delete();
        $zooid = implode(',', $request->input('concierge'));
        $zooid = array_map('intval', explode(',', $zooid));
        DB::table('concierge_userconfig')->insertGetId([
            'user' => $userID,
            'config' => json_encode($zooid)
        ]);
