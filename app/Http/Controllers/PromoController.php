...
class PromoController extends Controller
{
    ...
    public function saveinfo(Request $request, $id)
    {
        ...
        if ($getUser['admin']) {
            ...
            // Начало промо
            $promostart = $request->input('promostart');
            if ($promostart != '') {
            ...
            if ($request->file('media')) {
                ...
                DB::table('media')->insertGetId([
                    'a' => hexdec($folderA),
                    'b' => hexdec($folderB),
                    'name' => $fileName . '.' . $guessExtension,
                    'type' => $filetype
                ]);
                $request->file('media')->move(
                    env('MEDIA_FOLDER') . 'orig/' . $folderA . '/' . $folderB,
                    $fileName . '.' . $guessExtension
                );

                $promo['media'] = $folderA . '/' . $folderB.'/'.$fileName . '.' . $guessExtension;
            };
