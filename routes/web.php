...
$router->group(['middleware' => ['cors']], function () use ($router) {
    ...
    $router->post('/api/qrcode/check', 'PetQRCodeController@tokenCheck');
    $router->post('/api/qrcode/found-pet', 'PetQRCodeController@foundPet');
    $router->post('/bepaidinfo', 'BepaidController@bepaidinfo');
});
$router->group(['middleware' => ['cors', 'throttle:global']], function () use ($router) {
    $router->post('/user/login', 'UserController@login');
    $router->post('/user/code', 'UserController@checkCodeVerification');
    $router->post('/user/create', 'UserController@newUser');
    $router->post('/user/validate/phone', 'UserController@validateNewUser');
    $router->post('/user/validate/code', 'UserController@validateCode');
});
$router->group(['middleware' => ['cors', 'customAuth']], function () use ($router) {
    ...
    $router->post(
        '/services/missinganimal',
        'ServicesController@formMissingAnimal'
    ); // Сохранить номер телефона в профайле
    $router->post(
        '/services/mymissinganimal',
        'ServicesController@myMissingAnimal'
    ); // Сохранить номер телефона в профайле

    $router->post(
        '/services/missinganimalall',
        'ServicesController@missinganimalall'
    ); // Сохранить номер телефона в профайле
    $router->post(
        '/services/missinganimalgetinfo/{id}',
        'ServicesController@getinfo'
    ); // Сохранить номер телефона в профайле
    $router->post(
        '/services/missinganimalupd/{id}',
        'ServicesController@missinganimalupd'
    ); // Сохранить номер телефона в профайле


    $router->get('/api/qrcode', 'PetQRCodeController@index');
    $router->post('/api/qrcode/create', 'PetQRCodeController@create');
    ...
    $router->post('/pet-shops/list', 'PetShopController@list'); // Список вет клиник для карты
    $router->post('/pet-shops/edit/{id}', 'PetShopController@edit'); // Список вет клиник для карты
    $router->post('/pet-shops/update/{id}', 'PetShopController@update'); // Список вет клиник для карты
    $router->post('/pet-shops/create', 'PetShopController@create'); // Новая клиниа на карте




    $router->post('/configs/zooid', 'SubscribeController@configZooid'); // Узнать стоимость услуги
    $router->post('/configs/zoopolis', 'SubscribeController@configZooPolis'); // Узнать стоимость услуги
    ...
    $router->post(
        '/contest/new',
        'ContestController@fastCreateContest'
    ); // Быстрая регистрация конкурса по его названию
    $router->post(
        '/contest/namagement',
        'ContestController@managementContest'
    ); // Быстрая регистрация конкурса по его названию
    $router->post(
        '/contest/editdata',
        'ContestController@getEditContestData'
    ); // Загрузка информации о конкурсе, для его редактирования
    ...
    $router->post(
        '/console/client/savedataaddress/{clientID}',
        'UserController@saveProfileDataAddress'
    ); // Адрес в профайле
    $router->post(
        '/console/client/savedatapets/{clientID}',
        'UserController@saveProfileDataPets'
    ); // Информация о питомце в профайле

    $router->post(
        '/console/client/createnewclient',
        'ConsoleController@createNewUser'
    ); // Создание нового клиента по номеру телефона
